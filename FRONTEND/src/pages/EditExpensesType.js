...
const EditExpensesType = ({props}) => {
  ...
  const getExpensesType = async () => {
    const response = await axios.get(`${process.env.REACT_APP_BASE_URL}/expensestype/${id}`);
    setExpensesTypeName(response.data.classtypename);

  }
  const updateExpensesType = async (e) => {
    e.preventDefault();
    try {
      await axios.patch(`${process.env.REACT_APP_BASE_URL}/expensestype/${id}`, {
        expensestypename: expensestypename
      });
      navigate("/expensestype");
    } catch (error) {
      if(error.response) {
  ...
  return (
    <Layout>
        <div className='mt-10 ml-5'>
            <p className='font-bold text-3xl'>Expense Types</p>
            <p className='text-gray-400 text-2xl'>Edit Expense Type</p>
            <div className='bg-white h-[20rem] p-5  ml-1 mt-3 elevation'>
                <form onSubmit={updateExpensesType}>
                  
                  
                    <div className='mt-5'>
                      <label className='text-xl font-bold'>Expense Type</label>
