...
const ExpensesType = () => {  
  ...
  const deleteExpensesType = async (userId) => {
    await axios.delete(`${process.env.REACT_APP_BASE_URL}/expensestype/${userId}`);
    getExpensesTypes();
    navigate(0);
  }


  const [expensestypename, setExpensesTypeName] = useState("");
  const [msg, setMsg] = useState("");
  const saveExpensesType = async (e) => {
    e.preventDefault();
    try {
      await axios.post(`${process.env.REACT_APP_BASE_URL}/expensestype`, {
  ...
  return (
    <Layout >
      ...
      >
        <Box sx={style} >
          <p class="text-white text-xl p-3  bg-dark-purple w-full">Expenses Type DETAILS</p>
          <form onSubmit={saveExpensesType}>
            <div className='flex flex-row m-3 justify-around items-center'>
              ...
              <input type="text" id="languagename" class="bg-gray-50 border mb-4 border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-60 p-1.5 " 
                ...
                onChange={(e) => setExpensesTypeName(e.target.value)}  
              />
      ...
      <div className='m-3'>
        <div>
          <legend className='p-1 ml-3 text-xl text-blue-700'>Expenses Type</legend>
            <div className='flex'>
            ...
            <table className="w-[40rem]">
              ...
              <tbody className="text-gray-600  text-sm font-light">
                ...
                <tr key={item.uuid} className=" border-gray-400  hover:bg-gray-100 border-b-2">
                  ...
                  <div className="flex items-center justify-center">
                      <span className="font-medium uppercase">{item.expensestypename}</span>
                    </div>
                  </td>
                  <td className=" py-3 px-3 text-center  border border-dark-purple">
