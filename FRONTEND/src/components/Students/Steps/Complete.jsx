...
const Complete = () => {
  ...
  async function saveProspect(e) {
    ...
    try {
      setIsDisabled(true);
      await axios.post(
        `${process.env.REACT_APP_BASE_URL}/prospects`,
        {
          ...
          enddate: studentData.finaldate && studentData.finaldate,
          user: user.id,
        },
        {
  ...
  async function saveInvoice(e) {
    e.preventDefault();
    if (
      studentData.passportidg === undefined ||
      studentData.passportidg === ""
    ) {
      ...
      try {
        await axios.post(`${process.env.REACT_APP_BASE_URL}/invoice`, {
          prospectid: studentData.passportidg && studentData.passportidg + make,
          courselist:
            studentData.courseList.length > 0 ? studentData.courseList : {},
          examlist: studentData.examList.length > 0 ? studentData.examList : {},
          purchaselist:
            studentData.purchaseList.length > 0 ? studentData.purchaseList : {},
          accolist: studentData.accoList.length > 0 ? studentData.accoList : {},
          otherlist:
            studentData.otherFeeList.length > 0 ? studentData.otherFeeList : {},
          registration:
            studentData.registrationList.length > 0
              ? studentData.registrationList
              : {},
          ...
          invoicecode:
            studentData.passportidg && studentData.passportidg + make,
        });
    ...
    <div className="mt-1">
      <div className="flex flex-row item-center justify-end mr-10 mb-2">
        ...
        <div
          className={
            isDisabled
              ? `flex bg-gray-500  items-center shadow-2xl w-auto justify-center rounded-lg p-2`
              : `flex bg-primary items-center shadow-2xl w-auto justify-center rounded-lg p-2`
          }
        >
