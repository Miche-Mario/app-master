...
const CoursesPurchasesExam = () => {
  ...
  const [currencyvalue, setCurrencyValue] = useState(
    currency.length !== 0 ? currency.currencyvalue : 1
  );
  const [lecurrency, setLecurrency] = useState(
    currency.length !== 0 ? currency.lecurrency : "XOF"
  );

  const getExamPrice = async (e) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/examprice`,
        {
  ...
  const getAccomodaion = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/accomodations`
    );
  ...
  const getAccoPrice = async (e) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/accomodationprice`,
        {
  ...
  const getOtherFee = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/otherfee`
    );
  ...
  const getOtherFeePrice = async (e) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/otherfeeprice`,
        {
  ...
  const getItems = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/purchases`
    );
  ...
  const getPurchasePrice = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/purchaseprice`,
        {
 ...
 */
  ...
  const getCourseSubcourse = async (e) => {
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/getsubcourse`,
        {
          courseid: programId,
        }
      );
  ...
  const getCoursesPrice = async (e) => {
    ...
    try {
      const response = await axios.post(
        `${process.env.REACT_APP_BASE_URL}/getcoursesprice`,
        {
      ...
      response.data.response
        ? setPriceId(response.data.response.id)
        : setPriceId(response.data.id);
      response.data.response
        ? setPrice(response.data.response.price * currencyvalue)
        : setPrice(response.data.fullprice * currencyvalue);
      response.data.fullprice &&
        setPrice(response.data.fullprice * currencyvalue);

      response.data.fullprice &&
        setCourseDescription(response.data.response.description);
  ...
  const getAccoDiscounts = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/accodiscount`
    );
  ...
  if (accoList && accoList.length > 0) {
    if (
      accoDiscounts.length > 0 &&
      accoList[0].acotimes >= parseInt(accoDiscounts[0].from)
    ) {
  ...
  const getGroupDiscount = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/groupdiscount`
    );
  ...
  const getStudentDiscount = async (e) => {
    e.preventDefault();
    const response = await axios.post(
      `${process.env.REACT_APP_BASE_URL}/discountcode`,
      {
        code: lecode,
      }
    );
    response.data && response.data.pourcentage
      ? setStudentDiscount(response.data.pourcentage)
      : setStudentDiscount(1);
    response.data && response.data.id
      ? setStudentDiscountid(response.data.id)
      : setStudentDiscountid("");
  };
  ...
  const fixfee =
    courseList.length !== 0 ? registrationprice * currencyvalue : 0;
  ...
  if (NumberOfCourse <= 1) {
    ...
    for (let i = 1; i < NumberOfCourse; i++) {
      fee =
        groupdiscount !== 1
          ? (courseList[i].price * groupdiscount) / 100
          : courseList[i].price;
    ...
    total = Math.round(
      studentdiscount === 1
        ? courseList[0].price +
            (sumCoursePrice - courseList[0].price) -
            discount +
            sumExamPrice +
            newSumAccoPrice +
            sumPurchasePrice +
            ...
            fixfee
        : sumCoursePrice -
            (sumCoursePrice * studentdiscount) / 100 +
            sumExamPrice +
            newSumAccoPrice +
            sumPurchasePrice +
            fixfee +
  ...
  const getCurrencies = async () => {
    const response = await axios.get(
      `${process.env.REACT_APP_BASE_URL}/currency`
    );
      ...
      <div>
        <div className="m-5">
          <div>
            ...
            <select
              class="bg-blue-100 border border-gray-300 text-gray-900 text-xl p-2 focus:ring-blue-500 
          ...
          <div className="flex">
            <div className="mt-6">
              ...
              <select
                class="bg-blue-100 border border-gray-300 text-gray-900 text-xl p-2 
                ...
                id="lecourse"
              >
                <option value="33" className="text-md">
                  Our programme
                </option>
                {course.map((course) => (
                  <option className="text-xl" value={course.course_courseid}>
            ...
            <div className="m-5">
              <select
                ...
                onChange={(e) => {
                  ...
                  handleChangeSubCourse();
                }}
                id="lesubcourse"
              >
                <option></option>
                {options.map((option) => (
                  <option value={option.subcourse.id}>
                    {option.subcourse.subcoursename}
                  </option>
                ))}
              </select>
            </div>
        ...
        focus:border-blue-500 block w-[21rem]"
                  ...
                  <div className="flex items-center">
                    ...
                    <div className="ml-3  font-bold ">
                      <input
                        type="date"
                        id="dateofbirth"
                        class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 ml-3 block w-[13.5rem] p-2.5 "
                ...
                >
                  ...
                  <span className=" ml-2 text-lg text-green-600 font-medium">
                    {" "}
                    Add a course
                  </span>
                </button>
        ...
        focus:border-blue-500 block w-[32rem]"
            ...
            <select
              class="bg-blue-100 border border-gray-300 text-gray-900 mt-3 text-xl p-2 
        focus:ring-blue-500 
        focus:border-blue-500 block w-[20rem]"
              onClick={getPurchasePrice}
              onChange={(e) => {
                setPurchaseTimes(e.target.value);
                setPurchaseButtonActif(false);
              }}
            >
              ...
              <option value="9" className="text-md">
                9
              </option>
              <option value="10" className="text-md">
                10
              </option>
              <option value="11" className="text-md">
                11
              </option>
              <option value="12" className="text-md">
                12
              </option>
            </select>
          </div>
        ...
        focus:border-blue-500 block w-[32rem]"
                  ...
                  onChange={(e) => {
                    handleChangeAcco();
                    setAccoId(e.target.value);
                  }}
                  onClick={getAccoPrice}
                >
                  <option value="" className="text-md"></option>
                  {accomodation.map((acco) => (
                    <option className="text-xl" value={acco.id}>
                      {acco.accomodationname}
                    </option>
                  ))}
                </select>
              </div>
        ...
        focus:border-blue-500 block w-[32rem]"
                  id="lotherfee"
                  onChange={(e) => {
                    handleChangeOtherFee();
                    setOtherFeeId(e.target.value);
                  }}
                  onClick={getOtherFeePrice}
                >
                  <option value="" className="text-md"></option>
                  {otherfee.map((fee) => (
                    <option className="text-xl" value={fee.id}>
                      {fee.feename}
                    </option>
                  ))}
                </select>
        ...
        <div className="w-[43rem]">
          <div className=" bg-dark-purple w-full p-1">
            <div className="flex justify-end items-center">
              ...
              <span className="text-white ml-6 bg-red font-bold min-w-1 6 p-2 rounded-xl text-xl">
                {separator(total)}
              </span>
            </div>
          </div>
          <div>
            {courseList.length > 0 && (
              <div>
                <div className="w-full bg-gray-300 text-right p-2 font-semibold border-b-2">
                  {registrationList[0].registrationname} :{" "}
                  <span className="font-bold text-xl">
                    {registrationList[0].regir}
                  </span>
                </div>
                <table class="min-w-full shadow-lg border-collapse block md:table">
                  <thead class="block md:table-header-group">
                    <tr class="border border-gray-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
                      <th class=" bg-dark-purple p-2 w-10 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        N
                      </th>
                      <th class="bg-dark-purple w-48 p-2 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Course{" "}
                      </th>
                      <th class="bg-dark-purple p-2 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Subcourse{" "}
                      </th>
                      <th class="bg-dark-purple p-2 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Duration
                      ...
                      <th class="bg-dark-purple p-2 w-32 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Actions
                      </th>
                    </tr>
                  </thead>
                  <tbody class="block md:table-row-group">
                    {courseList.map((course, index) => (
                      <tr class="bg-gray-300 border-b-2 block md:table-row">
                        <td class="p-3 w-10  text-center font-medium block md:table-cell">
                          {index + 1}
                        </td>
                        <td class="p-3 w-48 text-center font-medium block md:table-cell">
                          {course.lecoursename}
                        </td>
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {course.lesubcoursename}
                        </td>
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {course.laduration}
                        </td>
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {course.lecurrency}{" "}
                          {course.price && separator(course.price)}
                        </td>
                        <td class="p-3 w-32 text-center font-medium block md:table-cell">
                          <div
                            onClick={() => {
                              deleteCourseFromList(course.uuid);
                              deleteRegistrationFromList(course.uuid);
                            }}
                            className=" cursor-pointer flex items-center p-2 shadow-md bg-white rounded-md"
                          >
                            <BiTrash
                ...
                <div className="mb-3 justify-end flex m-2">
                  <button
                    ...
                    className="justify-center flex items-center cursor-pointer p-1 shadow-lg rounded-lg font-semibold bg-redd text-white w-64"
                  >
                    Remove All
                  </button>
                </div>
                ...
                <table class="min-w-full shadow-lg border-collapse block md:table">
                  <thead class="block md:table-header-group">
                    <tr class="border border-gray-500 md:border-none block md:table-row absolute -top-full md:top-auto -left-full md:left-auto  md:relative ">
                      ...
                      <th class="bg-dark-purple p-2 w-48 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Exam{" "}
                      </th>
                      <th class="bg-dark-purple p-2 text-white font-bold md:border md:border-grey-500 text-center block md:table-cell">
                        Price
                  ...
                  <tbody class="block md:table-row-group">
                    {examList.map((exam, index) => (
                      <tr class="bg-gray-300 border-b-2 block md:table-row">
                        ...
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {" "}
                          {exam.lecurrency} {exam.examprice}
                        </td>
                        <td class="p-3 w-32 text-center font-medium block md:table-cell">
                          <div
                            onClick={() => deleteExamFromList(exam.uuid)}
                            className=" cursor-pointer flex items-center p-2 shadow-md bg-white rounded-md"
                          >
                            <BiTrash
                              style={{ fontSize: "20px", color: "red" }}
                  ...
                  <tbody class="block md:table-row-group">
                    {purchaseList.map((purchase, index) => (
                      <tr class="bg-gray-300 border-b-2 block md:table-row">
                        ...
                        <td class="p-3 w-48 text-center font-medium block md:table-cell">
                          {purchase.lepurchasename}
                        </td>
                        <td class="p-3 w-48 text-center font-medium block md:table-cell">
                          {purchase.purchasetimes && purchase.purchasetimes}
                        </td>
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {" "}
                          {purchase.lecurrency}{" "}
                          {purchase.purchaseprice &&
                            separator(purchase.purchaseprice)}
                        </td>
                        <td class="p-3 w-32 text-center font-medium block md:table-cell">
                          <div
                            onClick={() =>
                              deletePurchaseFromList(purchase.uuid)
                            }
                            className=" cursor-pointer flex items-center p-2 shadow-md bg-white rounded-md"
                          >
                            <BiTrash
                  ...
                  <tbody class="block md:table-row-group">
                    {accoList.map((acco, index) => (
                      <tr class="bg-gray-300 border-b-2 block md:table-row">
                        ...
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {acco.lecurrency}{" "}
                          {acco.accoprice && separator(acco.accoprice)}
                        </td>
                        <td class="p-3 w-32 text-center font-medium block md:table-cell">
                          <div
                            onClick={() => deleteAccoFromList(acco.uuid)}
                            className=" cursor-pointer flex items-center p-2 shadow-md bg-white rounded-md"
                          >
                            <BiTrash
                              style={{ fontSize: "20px", color: "red" }}
                  ...
                  <tbody class="block md:table-row-group">
                    {otherFeeList.map((fee, index) => (
                      <tr class="bg-gray-300 border-b-2 block md:table-row">
                        ...
                        <td class="p-3 text-center font-medium block md:table-cell">
                          {fee.lecurrency}{" "}
                          {fee.otherfeeprice && separator(fee.otherfeeprice)}
                        </td>
                        <td class="p-3 w-32 text-center font-medium block md:table-cell">
                          <div
                            onClick={() => deleteOtherFeeFromList(fee.uuid)}
                            className=" cursor-pointer flex items-center p-2 shadow-md bg-white rounded-md"
                          >
                            <BiTrash
                              style={{ fontSize: "20px", color: "red" }}
