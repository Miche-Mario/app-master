...
const InvoiceTableRow = ({items}) => {
  ...
  function separator(numb) {
    ...
    return (
        <>
        {items && items.registrationList.length > 0 && items.registrationList.map( (item, index) => 
        ...
        {items && items.examList.length > 0 && items.examList.map( (item, index) => 
            <View style={styles.row} key={index}>
                <Text style={styles.qty}>1</Text>
        ...
        {items && items.otherFeeList.length > 0 && items.otherFeeList.map( (item, index) => 
            <View style={styles.row} key={index}>
                <Text style={styles.qty}>1</Text>
                ...
                <Text style={styles.amount}>{item.lecurrency} {separator((item.otherfeeprice).toFixed(2))}</Text>
            </View>
        )}
        {items && items.accoList.length > 0 && items.accoList.map( (item, index) => 
            <View style={styles.row} key={index}>
                ...
                <Text style={styles.amount}>{item.lecurrency} {separator((item.accoprice * item.acotimes).toFixed(2))}</Text>
            </View>
        )}
        </> 
    )
